  ...
  const fetchData = async (page) => {
    ...
    try {
      const res = await axios({
        url: `https://aadrikainfomedia.com/auth/api/lams/v1/asset/get?limit=7&page=${page}&search=${debouncedSearch}&filter=${filter}&status=3`,
        method: "GET",
        headers: {
      ...
      // Filter data to include only items with status 3
      const filteredData =
        res?.data?.data?.data.filter((item) => item.status === 3) || [];
    ...
    <div className="px-4 overflow-x-hidden">
      ...
      <div className="p-4">
        <TitleBar
          ...
          titleText={"on Hold Application View"}
        />
      </div>
          ...
          {[...Array(totalPages)].map((_, index) => (
            <li key={index}>
              <button
                ...
                className={`flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-red border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white ${
                  currentPage === index + 1
