...
const FieldOfficer = () => {
  ...
  const fetchData = async (page) => {
    ...
    try {
      ...
      // Filter data to include only items with status 0
      console.log("response >>>  api", res?.data?.data);
      const filteredData =
        res?.data?.data?.data.filter((item) => item.status === 0) || [];
    ...
    <div className="px-4 overflow-x-hidden">
      ...
      <div className="p-4">
        <TitleBar
          ...
          titleText={"Home Application View"}
        />
      </div>
            ...
            <tbody>
              {filteredData?.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">{item?.ward_no}</td>
                  <td className="px-4 py-3">{item?.area}</td>
                  <td className="px-4 py-3" onClick={() => alert("To see documents, go to the view page")}>
