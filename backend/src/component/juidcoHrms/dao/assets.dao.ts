...
post = async (req: Request) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const validAssetType = asset_type ? asset_type.toLowerCase().trim().replace(/\s+/g, '') : '';
      ...
      const assetReq = await tx.assets_list.create({
        data: {
          ...
          is_drafted,
          status: 0,
          parent_land_id: parent_land_id ?? null, // 👈 set null if not provided,
          floor_data: {
            create: floorData?.map((floor: any) => ({
              ...
              details: {
                create: floor.details?.map((detail: any) => ({
                  ...
                  property_no: detail.property_no,
      ...
      // 4. Create related entries
      await tx.asset_fieldOfficer_req.create({
        data: { assetId: assetReq.id },
      });
...
getAll = async (req: Request) => {
  ...
  const isFieldOfficer = req.query.is_fieldofficer === 'true';
  ...
  const whereClause: any = {
    ...
    ...(search && {
      OR: [
        ...
        { plot_no: { contains: search, mode: 'insensitive' } },
    ...
    AND: [
      filter ? {
        OR: [
          { asset_type: { equals: filter, mode: 'insensitive' } },
          { type_of_land: { equals: filter, mode: 'insensitive' } },
        ],
      } : {},
      status || status === 0 ? { status } : {},
      land ? { type_of_land: { equals: land, mode: 'insensitive' } } : {},
      ward_no ? { ward_no: { equals: ward_no, mode: 'insensitive' } } : {},
      isFieldOfficer ? { is_drafted: false } : {},
    ],
  };

  // Count total matching
  const count = await prisma.assets_list.count({ where: whereClause });
  const totalPages = Math.ceil(count / limit);

  // Fetch paginated assets with relations
...
getAllbyId = async (req: Request) => {
  ...
  try {
    const assetGetbyId = await prisma.assets_list.findFirst({
      ...
      include: {
        parent_land: true, // <-- Fetch parent land data if exists
        floor_data: {
          include: {
            details: true,
          },
        },
        asset_field_officer_req: {
          include: {
            asset_checker_req: true,
          },
        },
      },
    });
    ...
    console.error("Error in getAllbyId DAO:", err);
    return {
      success: false,
      error: "Error fetching asset by ID",
    };
  }
};
